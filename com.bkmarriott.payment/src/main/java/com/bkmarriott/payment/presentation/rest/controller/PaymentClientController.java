...
public class PaymentClientController {
  ...
  public PaymentDto processPayment(@RequestBody PaymentRequestDto request) {
    return PaymentDto.from(paymentService.createPayment(CreatePaymentDto.from(request)));
  }

  @PatchMapping("/api/v1/payments/refund/{paymentId}")
  public PaymentDto refundPayment(@PathVariable Long paymentId) {
    return PaymentDto.from(paymentService.refundPayment(paymentId));
  }
}
